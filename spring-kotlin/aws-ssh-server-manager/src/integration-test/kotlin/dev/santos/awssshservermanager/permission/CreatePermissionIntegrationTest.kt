...
class CreatePermissionIntegrationTest : IntegrationTestBase() {
  ...
  fun `creates permission successfully`() {
    ...
    Assertions.assertThat(newPermission.hostGroupId).isEqualTo(createPermissionFixture.createPermissionDto.hostGroupId)
    Assertions.assertThat(newPermission.grantorId).isEqualTo(createPermissionFixture.createPermissionDto.grantorId)
